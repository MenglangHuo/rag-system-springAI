...
export const useChatStore = defineStore('chat', () => {
  ...
  function persist() {
    ...
    if (activeId.value) localStorage.setItem('klc:active', activeId.value)
  }
  ...
  function newChat() {
    const c: Conversation = { id: uid(), title: 'New conversation', messages: [], createdAt: Date.now(), updatedAt: Date.now() }
    conversations.value.unshift(c)
    activeId.value = c.id
    persist()
  }

  function selectChat(id: string) { activeId.value = id; persist() }
  ...
  async function sendMessage(text: string) {
    ...
    if (conv.messages.length === 1) conv.title = text.trim().slice(0, 48)
    const assistant: Message = { id: uid(), role: 'assistant', content: '', createdAt: Date.now(), streaming: true }
  ...
  function stop() { abortCtrl?.abort(); isStreaming.value = false; if (active.value) { const last = active.value.messages.at(-1); if (last) last.streaming = false } }

  async function regenerate() {
    if (!active.value || isStreaming.value) return
