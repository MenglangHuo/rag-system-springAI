/**
 * Streaming helper.
 * Default is a local SIMULATED stream so the UI works out of the box.
 * Swap `simulatedStream` for `sseStream` and point it at your backend
...
export async function simulatedStream(prompt: string, h: StreamHandlers) {
  const text = SAMPLE
...
export async function sseStream(
  endpoint: string,
  body: unknown,
  h: StreamHandlers & { onSessionId?: (id: string) => void },
) {
  try {
          ...
          if (eventData.length > 0) {
            ...
            if (chunk.startsWith('[SESSION:')) {
              ...
              if (end !== -1) {
                ...
                if (h.onSessionId) h.onSessionId(sessionId)
